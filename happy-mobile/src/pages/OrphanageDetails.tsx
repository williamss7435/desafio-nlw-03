...
export default function OrphanageDetails() {
  const routeParams = useRoute().params as OrphanageDetailsParams;
  
  const [orphanage, setOrphanage] = useState<Orphanage>();
  
  useEffect(() => {
    (async () => {
      const response = await happyService.get(`orphanages/${routeParams.id}`);
    ...
    <ScrollView style={styles.container}>
      <View style={styles.imagesContainer}>
        <ScrollView horizontal pagingEnabled>
          {orphanage.images.map(orphanage => {
            return (
              <Image 
                key={orphanage.id}
                style={styles.image} 
                source={{ uri: orphanage.url}} 
              />
            );
          })}
        </ScrollView>
